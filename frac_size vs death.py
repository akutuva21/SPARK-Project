...
# ax.scatter(three.iloc[:, 5], three.iloc[:, 2], c= 'b', s= 10, label = 'Linear-Quadratic')
...
# m = 2
# s = 8
# one, two, three = one.where(one.iloc[:, 5] < m), two.where(two.iloc[:, 5] < m), three.where(three.iloc[:, 5] < m) 
...
# ax.indicate_inset_zoom(subax1, edgecolor="black", linewidth = 5, alpha = 0.75, visible = True)
plt.tight_layout()
plt.show()